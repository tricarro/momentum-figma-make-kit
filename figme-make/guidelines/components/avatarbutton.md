...
**AvatarButton** is the **interactive** form of **Avatar**: same **photo / icon / initials / counter / presence** behavior, wrapped in a **button** (focusable, click, keyboard). For **display-only** avatars, use **`Avatar`**.

See also: [Avatar](./avatar.md).

Browse the [Momentum Components catalog](https://momentum.design/en/components/) for naming; Storybook documents props and examples. Official reference: [Storybook — AvatarButton / Docs](https://momentum.design/storybook-static/index.html?path=/docs/components-avatar-avatarbutton--docs). Optional: [AvatarButton / Example](https://momentum.design/storybook-static/index.html?path=/story/components-avatar-avatarbutton--example).
...
- **`onClick`**, **`disabled`**, **`softDisabled`**, **`type`**, etc.: as in **Buttonsimple** / Storybook (`softDisabled` keeps focus while looking disabled—you must still guard actions).
...
<AvatarButton initials="MD" size="88" ariaLabel="Open account menu"></AvatarButton>
```

(Add **`src`**, **`presence`**, etc. per design; use a real **`ariaLabel`** string.)
