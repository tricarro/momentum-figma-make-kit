# Components (Momentum + Figma Make)

How to use Momentum React components in Figma Make prototypes. **Install, CSS imports, providers, and theming** are in [setup.md](./setup.md) — this file focuses on component choice, the published export list, **icons** rules, and per-component deep dives.

## General usage

Import and use Momentum React components directly. Do not recreate interactive UI with native HTML when a Momentum component exists.
...
For props, examples, and checklists for a specific component, use the matching file in **`./components/`** — filenames are **lowercase** and match the export name (e.g. `Button` → `button.md`, `Appheader` → `appheader.md`, `Selectlistbox` → `selectlistbox.md`):

`./components/<lowercase-name>.md`
...
### Allowed sources and rendering

- **Only** the Momentum icon set shipped with `@momentum-design/components` / `@momentum-design/icons` is allowed. **Every** icon must be rendered with `<Icon name="…" />` and a `name` that is a real symbol from that set (per [Momentum design](https://momentum.design), component docs, or Storybook — not invented kebab-strings that “sound” right). Configure **IconProvider** at the app root per [setup.md](./setup.md#theme-and-icon-providers).
