 ...
 */
export const getElementRectWithMidPoint = (element) => {
 ...
 */
const orthogonalDisplacement = (ref, candidate, dirAxis) => dirAxis === 'horizontal' ? Math.abs(candidate.yMid - ref.yMid) : Math.abs(candidate.xMid - ref.xMid);
/**
 ...
 * @param dirAxis - Direction axis (horizontal or vertical)
 */
const orthogonalBias = (ref, dirAxis) => dirAxis === 'horizontal' ? ref.height / 2 : ref.width / 2;
/**
 ...
 * @param dirAxis - Direction axis (horizontal or vertical)
 */
const overlap = (ref, candidate, dirAxis) => dirAxis === 'horizontal'
 ...
 */
const alignBias = (ref, dirAxis, overlap) => dirAxis === 'horizontal' ? overlap / ref.height : overlap / ref.width;
/**
 ...
 */
const findTheShortestDistance = (ref, candidate, dirAxis, weights) => {
    const dx = candidate.xMid - ref.xMid;
    const dy = candidate.yMid - ref.yMid;
    const euclideanDistance = Math.sqrt(dx * dx + dy * dy);
    const displacement = orthogonalDisplacement(ref, candidate, dirAxis) + orthogonalBias(ref, dirAxis) * weights.orthogonalWeight[dirAxis];
 ...
 */
export const orderElementsByDistance = (activeElement, focusableElements, direction, weights) => {
    // No other focusable elements
    if (!activeElement || focusableElements.length === 0)
        return [];
    ...
    return focusableElements
        ...
        if (direction === 'left' && cr.right > reference.xMid)
            return acc;
        if (direction === 'right' && cr.left < reference.xMid)
            return acc;
        if (direction === 'up' && cr.bottom > reference.yMid)
            return acc;
        if (direction === 'down' && cr.top < reference.yMid)
            return acc;
        // Calculate distance
